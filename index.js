	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'Chernoff'//Nama Bot
                        const ownerName = 'TioChernoff<3'//Nama Owner
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, exemplo ${prefix}marvellogo TioChernoff Criador`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'neonime':
					...
					break  
					case 'bpink':
                  if (args.length < 1) return reply(`Entrada de texto\Exemplo : ${prefix} Chernoff Bot`)
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*[ CHERNOFF BROADCAST ]*\n\n${body.slice(4)}`)
						}
			...
			mus = body.slice(6)
               ...
               if (mus1.error) return reply(mus1.error)
				...
				case 'logowolf2':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf chernoff|bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case '8bit':
                                        ...
                                        var bi2 = bi.split("|")[0];
                                        var bit = bi.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}8bit Chernoff|Anjay')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        reply(mess.wait)
